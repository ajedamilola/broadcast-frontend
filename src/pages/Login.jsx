    ...
    if (window.ethereum) {
      try {
        ...
        if (network.chainId != 421614) {
          Report.warning("Network Not found", "Arbitrum testnet not found please add it or switch to it")
          setConnecting(false)
          return;
        }
        ...
        Report.failure("An Error occured", "Unable to signin into wallet please try again")
        console.log(error)
      }
    } else {
      Report.warning("Unable to connect wallet", "Try getting a wallet plugin like metamask to proceed")
    }
    setConnecting(false)
  }
  return (
    <div className='font-montserrat min-h-screen flex items-center justify-center bg-gradient-to-r from-[#0389c9] to-[#03a9f4]'>
      <div className='bg-white p-8 rounded-lg shadow-2xl transform hover:scale-105 transition-transform duration-300'>
        ...
        <button className='w-full bg-[#0389c9] hover:bg-[#026d9e] text-white font-bold py-3 px-4 rounded-lg transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg disabled:opacity-70' onClick={handleConnect} disabled={connecting}>
